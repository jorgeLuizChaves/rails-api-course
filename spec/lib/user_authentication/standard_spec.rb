...
describe UserAuthentication::Standard do
  describe '#perform' do
    ...
    shared_examples_for 'invalid authentication' do
      before { user }
      it 'should rainse AuthenticationError' do
        expect{ subject }.to raise_error(UserAuthentication::Standard::AuthenticationError)
        expect(authenticator.user).to be_nil
      end
    end

    context 'when invalid login' do
      let(:user) {create :user, login: 'abc', password: 'secret'}
